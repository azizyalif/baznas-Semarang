<div class="flex flex-col col-span-full sm:col-span-6 bg-white dark:bg-gray-800 shadow-xs rounded-xl">
    <header class="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60">
        <h2 class="font-semibold text-gray-800 dark:text-gray-100">Permohonan Berdasarkan Program</h2>
    </header>
...
document.addEventListener('DOMContentLoaded', function () {
    fetch('/json-get-data-by-program')
        .then(response => response.json())
        .then(data => {
            const programLabels = data.program.labels;
            const programData = data.program.data;
            const colors = [
